...
function CircularProgressBar({wid, value, setSettings, timeLeft, setTimeLeft, isPaused, setIsPaused}) {
  
  let da = Math.PI * wid * 2;
  let p = da / 100;
  let st = (100 - (((value * 60)- timeLeft)* 100 / (value*60))) * p;
  
  return (
    <div className='CPB'>
        <svg width={wid*2 + 40} height={wid *2 + 40} viewBox={`0 0 ${wid*2 + 40} ${wid*2 + 40}`}>
          <circle cx = {wid + 20} cy = {wid + 20} r = {wid}          
          ...
          <text x = '50%' y = '59%' textAnchor='middle' className='Circle-text'>
            {`${Math.floor(timeLeft/60)}:${(timeLeft%60 < 10 ? ('0' + timeLeft%60) : timeLeft%60)}`}
          </text>
        </svg>
        <div className='bCon'>
          {isPaused? <button onClick = {() => {setIsPaused(false)}}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
              <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm14.024-.983a1.125 1.125 0 0 1 0 1.966l-5.603 3.113A1.125 1.125 0 0 1 9 15.113V8.887c0-.857.921-1.4 1.671-.983l5.603 3.113Z" clipRule="evenodd" />
            </svg>
          </button>:
          <button disabled = {timeLeft=== 0}onClick = {() => {setIsPaused(true)}}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
